...
use crate::models::{ChapterResponse, Subject, SubjectCreate, SubjectResponse, SubjectType, SubjectUpdate};
use chrono::Utc;
use uuid::Uuid;
...
pub async fn upload_subject(
    ...
    while let Some(field) = multipart
        .next_field()
        .await
        .map_err(|e: axum::extract::multipart::MultipartError| AppError::BadRequest(format!("Multipart error: {}", e)))?
    {
        let name = field.name().unwrap_or_default().to_string();
        match name.as_str() {
            "subject_create" => {
                let data = field.bytes().await.map_err(|e: axum::extract::multipart::MultipartError| AppError::BadRequest(format!("Error reading subject_create: {}", e)))?;
                let sc: SubjectCreate = serde_json::from_slice(&data).map_err(|e: serde_json::Error| AppError::BadRequest(format!("Invalid subject_create JSON: {}", e)))?;
                title = Some(sc.title);
                subject_type = Some(sc.r#type);
                is_public = sc.is_public;
            }
            "title" => {
                title = Some(field.text().await.map_err(|e: axum::extract::multipart::MultipartError| AppError::BadRequest(format!("Error reading title: {}", e)))?);
            }
            "type" => {
                let t = field.text().await.map_err(|e: axum::extract::multipart::MultipartError| AppError::BadRequest(format!("Error reading type: {}", e)))?;
            ...
            "is_public" => {
                let val = field.text().await.map_err(|e: axum::extract::multipart::MultipartError| AppError::BadRequest(format!("Error reading is_public: {}", e)))?;
            ...
            "file" => {
                ...
                file_data = Some(
                    field
                        .bytes()
                        .await
                        .map_err(|e: axum::extract::multipart::MultipartError| AppError::BadRequest(format!("Error reading file: {}", e)))?
                        .to_vec(),
                );
            }
            _ => {}
    ...
    let (title, subject_type, file_data, file_name) = match (title, subject_type, file_data, file_name) {
        (Some(t), Some(st), Some(fd), Some(fnm)) => (t, st, fd, fnm),
        _ => return Err(AppError::BadRequest("Missing required fields (title, type, and file)".to_string())),
    };
    ...
    let bg_tasks = state.bg_tasks.clone();
    let uid = current_user.id.clone();
    let sid = subject_id.clone();
    let fp = file_path.clone();

    tokio::spawn(async move {
        bg_tasks.process_subject_bg(uid, sid, fp).await;
    });
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Successfully fetched subject chapters", body = [ChapterResponse]),
        (status = 404, description = "Not found")
    ),
    tag = "Subjects"
)]
pub async fn get_subject_chapters(
    ...
    let response = chapters.into_iter().map(ChapterResponse::from).collect();
...
pub async fn update_subject(
    ...
    let mut subject = match state.db.get_subject(&subject_id).await {
        Ok(Some(s)) => s,
        _ => return Err(AppError::NotFound("Subject not found".to_string())),
    };

    if current_user.id != subject.user_id {
        return Err(AppError::Forbidden("You do not have permission to update this subject".to_string()));
    }
...
pub async fn delete_subject(
    ...
    if current_user.id != subject.user_id {
        return Err(AppError::Forbidden("You do not have permission to delete this subject".to_string()));
    }
