...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  return (
    ...
    >
      {isLoading ? (
        <span className="absolute flex left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
      ...
      <span className={isLoading ? 'opacity-0' : 'flex items-center opacity-100'}>{children}</span>
    </button>
  );
};